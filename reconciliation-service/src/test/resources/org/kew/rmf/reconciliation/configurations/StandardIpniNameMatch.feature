...
Feature: Reconcile against IPNI, using a standard configuration (not to lax, not too rigid).
	...
	Scenario: Exact match, at various ranks.
		When I query for
			| queryId | epithet_1      | epithet_2    | epithet_3 | basionymAuthor | publishing_author                       |
			| 11      | Artemisia      | absinthium   |           |                |                                         |
			| 12      | Artemisia      | absinthium   |           |                | L.                                      |
			| 13      | Abrotanella    | forsteroides |           | F.Muell.       | Benth.                                  |
		...
		Then the results are
			...
			| 11      | 1056033-2 179148-1 179149-1 179151-1 300106-2 60439431-2 926141-1 |                                                          |
	...
	Scenario: Single string queries
		When I query with only a single string for
			...
			| 43      | Abrotanella forsteroides (F.Muell.) Benth.                      |
		...
		Then the results are
			| queryId | results                                                           | remarks                                                  |
			| 41      | 1056033-2 179148-1 179149-1 179151-1 300106-2 60439431-2 926141-1 |                                                          |
